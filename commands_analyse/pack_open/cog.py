...
class pack_open(commands.Cog, name='pack_open'):
	def __init__(self, bot: commands.Bot):
		self.bot = bot

	@nextcord.slash_command(description="Display list of openers for a pack.", dm_permission=False, force_global=True)
	async def pack_open(self, interaction: nextcord.Interaction, pack: str = nextcord.SlashOption(
        name="pack", description="Pack's name or ID", required=True
    ), player: str = nextcord.SlashOption(
        name="player", description="Get number for a specific player", required=False,
    ),):
		await interaction.response.defer(with_message=True)

		print ("pack_open", pack)
		tID = pack
		sPlayer = "elindos"
		...
		mMax = 10
		(tPacks, tPack_owners, tPack_nbs, tPack_open, tPacks_n) = lib_analyze.db_Pack_GetOwners( tID, "opened", mMax, sPlayer=sPlayer, mysql=mysql )
